...
class CASAWorkflow(object):
    ...
    def generate_dax(self):
        ...
        for x in range(31):
            ...
            d3_job.uses(pr_file, link=Link.INPUT)
            d3_job.uses(pr_geojson, link=Link.OUTPUT, transfer=True, register=False)
            d3_job.profile("pegasus", "label", "label_"+str(x))
            dax.addJob(d3_job)
